...
def bold_text(node):
    ...
    new_node = ""
    matches = re.split(
        r"([a-zA-Z" + accentedCharacters + "-]+’?[a-zA-Z" + accentedCharacters + "-]+|[a-zA-Z" + accentedCharacters + "-])",
        node)
...
def get_filelist(folder_path):
    ...
    for line in open(content_file, encoding='utf-8'):
        # print("###################")
        if "application/xhtml+xml" in line:
            # print(line)
            x = line.split("href")[1].split("\"")
            for y in x:
                # print(y)
                if "htm" in y:
                    # print("file: "+folder_path+path_add+y)
                    filelist.append(folder_path + "/" + path_add + y)
                    break
...
for book in epub_list:
    ...
    for file in filelist:
        ...
        soup.select_one("style").append(
            "b{font : inherit;font-family : inherit;font-size : inherit;font-style : inherit;font-variant : inherit;font-weight : bold;}")
    ...
    if os.path.exists("./output/" + book.split(".epub")[0] + " (b).epub"):
        os.remove("./output/" + book.split(".epub")[0] + " (b).epub")
